        ...
        <Stack direction="column">
            <Stack direction="row" spacing={4} ml={8}>
                {Object.entries(tagInfo).map(([key, value]) => {
                    return (
                        <SongTag tagName={key} tagData={value}/>
                    )
                })}
            </Stack>
                ...
                renderInput={(startProps, endProps) => (
                    <React.Fragment>
                        ...
                        <TextField {...startProps} 
                            InputProps={{ classes: { input: classes.dateRange }, disableUnderline: true }} 
                            variant="standard"
                        />
                        <Box sx={{ mx: 4 }}> to </Box>
                        <TextField {...endProps} 
    ...
    return (
        <Stack direction="column" mb={4}>
            ...
            <Typography sx={{ fontSize: 12, mt: 2 }}>
                {numPerformances.stat + " " + numPerformances.statName + (numPerformances.stat > 1 ? "s" : "")}            
            </Typography>
            <Typography sx={{ fontSize: 12 }}>
                {numRequests.stat + " " + numRequests.statName + (numRequests.stat > 1 ? "s" : "") + " (" + Math.round((numRequests.stat / numPerformances.stat) * 100) + "%)"}
            </Typography>
            <Typography sx={{ fontSize: 12 }}>
                {numPlayers.stat + " " + numPlayers.statName + (numPlayers.stat > 1 ? "s" : "")}
            </Typography>
            <List dense={true}>
                {cmStats.chimesmasters.map((element) => {
                    const findSong = element.songs.filter(e => e.song === "La La Land");
                    const cmStr = findSong.length > 0 ? 
                        (<ListItem dense={true} sx={{ mb: -4, mt: -2 }}>
                            <CircleIcon sx={{ width: 8, color: "#006699", mr: 2 }}/>
                            <ListItemText 
                                sx={{ color: "#006699" }} 
                                primary={element.name + " (" + findSong[0].stats.performed + ")"}
                            />
                        </ListItem>) : 
                        null;
                    return (cmStr)
    ...
    const getDates = () => {
        ...
        cmStats.chimesmasters.map((cm) => {
            ...
            if (specificSong.length > 0) {
                specificSong[0].stats.date.map((day) => { dates.push(day + ": " + cm.name); })
            }
        })
        return dates;
    }
