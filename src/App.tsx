...
const App = () => {
  ...
  return (
    <ThemeProvider theme={theme}>
      ...
      <Box
        sx={{position:'fixed', left:posleft, top:126, zIndex: theme.zIndex.drawer+1,
        transform:'rotate(90deg)', transformOrigin:`${buttonOffsetX} 34px`,
        ...
        transition: (logOpen) ?  'left 225ms cubic-bezier(0.0, 0, 0.2, 1) 0ms' : 'left 195ms cubic-bezier(0.4, 0, 0.6, 1) 0ms'
        }}
      >
        <Button 
          disableRipple
          onClick={toggleOpen}
          variant='contained'
          sx={{position:'relative', left:0, zIndex: theme.zIndex.drawer+1,
            backgroundColor:'primary.light',
            color:'primary.dark',
            textTransform:'none', lineHeight:'18px',
            mx:buttonOffsetX,
            borderRadius:'4px 4px 0px 0px',
            "&.MuiButton-root": {
              padding:'8px 12px', // not sure why this works but doing it in the sx prop doesn't
              ":hover": {
                backgroundColor:'primary.light', fontWeight:'bold',
        ...
        <Box sx={{position:'relative', height:buttonOffsetY, width:'100%', zIndex: theme.zIndex.drawer+2, 
          color:'primary.light', backgroundColor:'primary.light', boxShadow:'none'
        }}>
        </Box>
      </Box>
